...
// Add testing mode handling
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.action === "getTestingMode") {
        chrome.storage.local.get("inTestingMode", function (data) {
            sendResponse({ inTestingMode: data.inTestingMode || false });
        });
        return true;
    }
});

// Add testing mode to suggestion timing
function getNextSuggestionTime() {
    return new Promise((resolve) => {
        chrome.storage.local.get("inTestingMode", function (data) {
            const inTestingMode = data.inTestingMode || false;
            if (inTestingMode) {
                // In testing mode, show suggestions more frequently
                resolve(5000 + Math.random() * 5000); // 5-10 seconds
            } else {
                // Normal mode
                resolve(5 * 60 * 1000 + Math.random() * 5 * 60 * 1000); // 5-10 minutes
            }
        });
    });
}

// Update showSuggestion function to include testing mode
function showSuggestion(tabId, isProductive) {
    chrome.storage.local.get("inTestingMode", function (data) {
        const inTestingMode = data.inTestingMode || false;
        chrome.tabs.sendMessage(
            tabId,
